...
public class NCIISUtil {
    ...
    public static NCID parseNCID(String rawCID, boolean validate) {
        rawCID = StringUtils.trim(rawCID);
        if (StringUtils.isEmpty(rawCID)) {
            return null;
        }
        ...
        if (matcher.find()) {
            ...
            if (validate) {
                if (!ncid.isValid()) {
                    throw new BaseBusinessException(BaseErrorCode.ID_NO_ERROR);
                }
            }
    ...
    public static boolean validateNCID(NCID ncid) {
        ...
        return checkCodeRaw.equalsIgnoreCase(checkCode);
    }
